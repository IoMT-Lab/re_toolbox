...
enum Args {
    ...
    FromGhidra {
        ...
        /// Path to output file for Ghidra variable map
        #[clap(long)]
        output_varmap: Option<PathBuf>,
    },
}
...
fn main() {
    ...
    match args {
        Args::FromGhidra {
            ...
            let var_map_path = output_varmap.unwrap_or_else(|| PathBuf::from("varmap.json"));
            let serializable_types =
                structuredtypes.serialize(&vars, var_map_path.to_str().unwrap());
